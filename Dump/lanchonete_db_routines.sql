...
/*!50001 VIEW `vw_pedidos` AS select `p`.`data` AS `data`,`p`.`id` AS `id`,`c`.`nome` AS `nome`,`p`.`end_entrega` AS `end_entrega`,`p`.`total` AS `total` from (`pedidos` `p` left join `clientes` `c` on((`p`.`id_cliente` = `c`.`id`))) where (`p`.`total` > 0) */;
